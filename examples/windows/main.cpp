// campello_renderer Windows example — GLFW window + DirectX 12 (via campello_gpu).
//
// Responsibilities:
//   - Creates a GLFW window (Win32) and a campello_gpu Device that owns its swapchain.
...
//     rather than continuously — see App::needsRedraw and main()'s
//     glfwWaitEvents()/glfwWaitEventsTimeout() usage, mirroring the macOS/Linux
//     examples' identical pattern.
//   - Handles mouse drag/scroll for orbit/pan/zoom camera control.
//   - Mirrors examples/linux/main.cpp almost line-for-line; the only real
//     difference is how the campello_gpu Device is created — campello_gpu's
//     DirectX backend just wants a raw HWND (Device::createDefaultDevice()
//     queries the window's client rect itself), unlike Linux's Wayland path,
//     which needs a display+window+api struct — see Device::createDefaultDevice()'s
//     doc comment / src/directx/device.cpp's createDeviceData() in campello_gpu.

// NOMINMAX: windows.h (pulled in by glfw3native.h below) otherwise #defines
// min/max as function-like macros, which corrupts every std::min/std::max
// call site — including inside vector_math's own headers — with a "too many
// arguments" syntax error at the call, not at windows.h's #define.
#define NOMINMAX
// No OpenGL/D3D headers needed from GLFW itself — campello_gpu owns the D3D12
// device/swapchain entirely once handed the HWND.
...
// ---------------------------------------------------------------------------
// ViewMode name/hotkey table — mirrors examples/linux/main.cpp exactly (same
// plain-key layout), just built against the same GLFW key constants.
// ---------------------------------------------------------------------------
...
struct App {
    ...
    // On-demand rendering (mirrors examples/linux/main.cpp's identical
    // pattern): starts true so the first frame renders, then only redraws in
    // response to an actual change (interaction, resize, load, or a playing
    // animation) instead of every loop iteration — see main()'s
    // glfwWaitEvents()/glfwWaitEventsTimeout() usage, which additionally lets
    // the process sleep with ~0% CPU while truly idle instead of polling on a
    // fixed timer.
...
static void framebufferSizeCallback(GLFWwindow *window, int /*width*/, int /*height*/) {
    // Deliberately does NOT call renderer->resize() itself — Win32 dispatches
    // WM_SIZE (and thus this callback) synchronously, many times per second,
    // from *inside* the OS's own modal live-resize loop while the outer
    // glfwWaitEvents() call in main()'s loop stays blocked on the call stack;
    // app.frame() (and thus this app's own code) doesn't run again until
    // that modal loop actually returns. Calling resize() here on every tick
    // used to create a brand-new depth Texture (and DSV) per tick with no
    // intervening frame ever submitted to let campello_gpu's DSV-heap slot
    // reclamation run (it's deferred until a submitted frame's fence
    // confirms the GPU is done) — a few hundred WM_SIZE ticks from one
    // continuous drag exhausted the DSV heap and the D3D12 debug layer
    // fail-fast-terminated the process (no crash dialog, no WER report —
    // confirmed via cdb.exe: "CreateDepthStencilView: ... does not refer to
    // a location in a descriptor heap" immediately followed by
    // RaiseException(STATUS_FATAL_APP_EXIT)). frame()'s own throttled resize
    // check (queries glfwGetFramebufferSize() directly, compares against its
    // cached fbWidth/fbHeight) already handles the actual resize exactly
    // once things settle — this callback only needs to make sure a redraw
    // gets scheduled.
    auto *app = static_cast<App *>(glfwGetWindowUserPointer(window));
    if (!app) return;
    app->needsRedraw = true;
    // Setting needsRedraw alone doesn't wake a blocked glfwWaitEvents() —
    // the OS's modal live-resize loop consumes every WM_SIZE on its own
    // nested message pump, so by the time it exits (drag released) there's
    // nothing left on the outer queue for our main-loop glfwWaitEvents()
    // call to wake up on; it just goes back to sleep until some unrelated
    // later event (e.g. the user happens to move the mouse) finally lets it
    // return. From the user's perspective this looked exactly like a 1-2s
    // "recreating the viewport" stall after releasing the mouse — it was
    // actually an already-finished resize just sitting undisplayed,
    // mistaken for slow GPU work. glfwPostEmptyEvent() forces an immediate
    // wake so the final size renders right away.
    glfwPostEmptyEvent();
}
...
static void dropCallback(GLFWwindow *window, int count, const char **paths) {
    ...
    fprintf(stderr, "campello_renderer_windows: dropped file(s) not a recognized .gltf/.glb "
...
static void cursorPosCallback(GLFWwindow *window, double x, double y) {
    ...
    // GLFW's cursor Y grows downward, matching Win32's own client-area convention —
    // see examples/linux/main.cpp's cursorPosCallback for the (unrelated) Cocoa
    // sign flip this doesn't need.
...
void App::printHelp() const {
    printf("campello_renderer -- Windows example\n");
...
bool App::init() {
    ...
    if (!glfwInit()) {
        fprintf(stderr, "campello_renderer_windows: failed to initialize GLFW\n");
        return false;
    }

    // No D3D context requested from GLFW — campello_gpu owns the DXGI
    // swapchain/D3D12 device itself once handed the HWND below.
    glfwWindowHint(GLFW_CLIENT_API, GLFW_NO_API);
    glfwWindowHint(GLFW_RESIZABLE, GLFW_TRUE);

    window = glfwCreateWindow(1280, 720, "campello_renderer -- Windows", nullptr, nullptr);
    if (!window) {
        fprintf(stderr, "campello_renderer_windows: failed to create window\n");
        glfwTerminate();
        return false;
    }
    ...
    // campello_gpu's DirectX backend just wants a raw HWND — it queries the
    // window's client rect itself for the initial swapchain size (see
    // createDeviceData() in campello_gpu's src/directx/device.cpp), unlike
    // Linux's Wayland path which needs an explicit display+window+size struct.
    HWND hwnd = glfwGetWin32Window(window);
    device = gpu::Device::createDefaultDevice(hwnd);
    if (!device) {
...
void App::loadModel(const std::string &path) {
    ...
    if (!readFile(path, bytes)) {
        fprintf(stderr, "campello_renderer_windows: could not read %s\n", path.c_str());
        return;
    }
    ...
    if (!asset) {
        fprintf(stderr, "campello_renderer_windows: failed to parse %s\n", path.c_str());
        return;
    }

    printf("campello_renderer_windows: loaded %s\n", path.c_str());
...
void App::loadEnvironmentMapFile(const std::string &path) {
    ...
    if (tex) {
        ...
        fprintf(stderr, "campello_renderer_windows: failed to load environment map %s\n", path.c_str());
    }
}
...
int main(int argc, char **argv) {
    // Unbuffer stdout so status/help output (device name, loaded model,
    // hotkey feedback) shows up promptly even when not attached to a TTY,
    // instead of sitting in a fully-buffered pipe until exit. _IOLBF was
    // tried first (matching examples/linux/main.cpp's/macOS's equivalent
    // line-buffering approach) but Windows' UCRT treats _IOLBF the same as
    // _IOFBF (full buffering) for any stream that isn't an actual console —
    // confirmed by redirecting stdout to a file: "loaded X"/"Bounds:"/
    // "Animations found:" printf lines sat unflushed indefinitely (the app
    // was actually running fine — std::cerr-based D3D12 debug-layer
    // messages appeared immediately since cerr is unbuffered by default,
    // while every stdout printf line silently vanished into a pipe-sized
    // buffer), which looked identical to a genuine hang. _IONBF sidesteps
    // this entirely (and its buffer/size args are ignored, so it also
    // sidesteps the separate 2 <= size <= INT_MAX UCRT assertion that a
    // NULL/0 buffer+size pair used to hard-crash on with _IOLBF/_IOFBF —
    // that crash looked identical to a device-creation crash until a real
    // debugger (cdb.exe) pointed straight at this line).
    ...
    // Release the Renderer/Device (and the DXGI swapchain they own, which
    // wraps the window's HWND) before tearing down GLFW/destroying the window.
    app.renderer.reset();
    app.device.reset();
